...
public class Activity_TakePhoto extends AppCompatActivity implements View.OnClickListener {
...
//    private boolean mFlashSupported;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mFile = new File(getCacheDir(), "captured.jpg");
        mTextureView = (AutoFitTextureView) findViewById(R.id.媒体呈现区);
...
//                        new CompareSizesByArea());
                Size largest = 选择照片尺寸(map.getOutputSizes(ImageFormat.JPEG));
...
//                mFlashSupported = available == null ? false : available;
    ...
    private Size 选择照片尺寸(Size[] 可选尺寸) {
        Size 某一尺寸;
        for (int i = 可选尺寸.length - 1; i >= 0; i--) {
            某一尺寸 = 可选尺寸[i];
            if (某一尺寸.getHeight() >= 1000 || 某一尺寸.getWidth() >= 1000) {
                return 某一尺寸;
            }
        }
        return new Size(0, 0);
...
//                                setAutoFlash(mPreviewRequestBuilder);
    ...
    private void takePicture() {
        File 文件 = new File(mFile.getAbsolutePath());
        if (文件.exists()) {
            文件.delete();
        }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.按钮_发送:
                File 文件 = new File(mFile.getAbsolutePath());
                if (文件.exists()) {
                    机器人.发送照片2(mFile.getAbsolutePath());
                    finish();
                } else {
                    takePicture();
                }
                break;
            case R.id.按钮_取消:
                文件 = new File(mFile.getAbsolutePath());
                if (文件.exists()) {
                    文件.delete();
                }
                finish();
            ...
            case R.id.文字_前后摄像头:
                ...
                if (!前置摄像头) {
                    前置摄像头 = true;
                    ((TextView)view).setText(界面文字.获取(51, "前置摄像头"));
                } else {
                    前置摄像头 = false;
                    ((TextView)view).setText(界面文字.获取(50, "后置摄像头"));
                }
                openCamera(mTextureView.getWidth(), mTextureView.getHeight());
                break;
        }
