...
public class Adapter_TaskName extends BaseAdapter {
    ...
    public Adapter_TaskName(Context 场景, List<? extends Map<String, ?>> 列表项, int 项布局, String[] From, int[] To) {
        this.列表项 = 列表项;
        this.项布局 = 项布局;
        this.From = From;
        this.To = To;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = 布局扩充器.inflate(项布局, parent, false);
        }
        final Map 某一项 = 列表项.get(position);
        if (某一项 != null) {
            for (int i = 0; i < To.length; i++) {
                View 控件 = convertView.findViewById(To[i]);
                if (控件 != null) {
